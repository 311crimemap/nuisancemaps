...
resource "hcloud_server" "db" {
  ...
  name        = format(
    ...
    count.index == 0 ? var.primary_server_type : var.replica_server_type,
    "${var.class_id}-${count.index}"
    )

  server_type = count.index == 0 ? var.primary_server_type : var.replica_server_type
  ...
  labels = merge(
    var.additional_labels,
    {
      ...
      # deploy k3s
      # label creates ansible group: label_node_server / label_node_agent
